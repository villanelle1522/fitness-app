...
Provide the response strictly as a JSON object matching the requested schema containing "mealName" and "items".`;
          ...
          config: {
            ...
            responseSchema: {
              ...
              properties: {
                mealName: {
                  type: "STRING" as any,
                  description: "A short, descriptive, professional name of the overall meal in Traditional Chinese (Taiwan syntax). Max 15 chars.",
                },
                items: {
                  type: "ARRAY" as any,
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      price: {
                        type: "NUMBER" as any,
                        description: "Estimated price in NTD ($) if applicable. Set to 0 if unknown.",
                      },
                    },
        ...
        if (text) {
          ...
          if (itemsArray.length > 0) {
            const itemsWithId = itemsArray.map((item: any, i: number) => ({
              id: Date.now() + i,
              name: item.name || "未命名食物",
              kcal: Number(item.kcal) || 0,
              protein: Number(item.protein) || 0,
            ...
            if (!Array.isArray(data) && data?.mealName) {
              setGroupName(data.mealName);
      ...
      if (response.ok && data.result) {
        ...
        const itemsArray = Array.isArray(resultObj) ? resultObj : (resultObj.items || []);
        const itemsWithId = itemsArray.map((item: any, i: number) => ({
          id: Date.now() + i,
          name: item.name || "未命名食物",
          kcal: Number(item.kcal) || 0,
        ...
        if (resultObj && resultObj.mealName) {
          setGroupName(resultObj.mealName);
        } else if (inputText.trim()) {
          setGroupName(inputText.trim().slice(0, 15));
        } else {
          setGroupName("AI 影像分析餐點");
    ...
    ...
      ...
      {parsedItems.length > 0 && (
        <div className="space-y-4 mt-2">
                  ...
                  <div className="w-[60px]">
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        className="bg-transparent border-b border-zinc-800 focus:border-indigo-500 text-xs text-zinc-400 text-right w-full focus:outline-none pb-0.5"
                        value={item.price || ""}
                        onChange={(e) => handleParsedItemChange(idx, "price", e.target.value)}
                        placeholder="金額"
                      />
          ...
          <div className="bg-zinc-900/30 p-5 border border-zinc-800/60 rounded-2xl space-y-5 text-xs">
            {/* Custom group name */}
            <div className="flex flex-col gap-2">
              <span className="text-zinc-500 font-bold text-[10px] uppercase tracking-wider">餐點名稱 (選填)</span>
              <input
                type="text"
                className="bg-zinc-900/50 border border-zinc-800/60 rounded-xl px-3 py-2.5 text-zinc-200 w-full focus:outline-none focus:ring-1 focus:ring-indigo-500 placeholder-zinc-600"
                placeholder="例：超商套餐、公司午餐 (將多個品項設為單一餐點)"
                value={groupName}
            ...
            <div className="flex flex-col gap-2">
              <span className="text-zinc-500 font-bold text-[10px] uppercase tracking-wider">新增至用餐時段</span>
              <div className="flex gap-1.5 w-full">
                {["早餐", "午餐", "晚餐", "點心"].map((cat) => (
                  <button
                    key={cat}
                    onClick={() => setSelectedCategory(cat)}
                    className={`flex-1 py-2 px-2 rounded-xl font-bold text-center transition-all cursor-pointer ${
                      selectedCategory === cat
                        ? "bg-indigo-500/20 border border-indigo-500/50 text-indigo-400 shadow-sm"
                        : "bg-zinc-900/50 text-zinc-400 border border-zinc-800/60 hover:text-zinc-300"
                    }`}
                  >
                    {cat}
                  </button>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-zinc-800/60">
              <label className="flex items-center gap-2 text-zinc-400 select-none cursor-pointer">
                <input
                  ...
                  className="rounded bg-zinc-900 border-zinc-800 text-indigo-500 focus:ring-0 focus:ring-offset-0 w-4 h-4 accent-indigo-500 cursor-pointer"
                />
                <span className="font-bold">同時儲存至您的「我的食物庫」方便下次快速加選</span>
              </label>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full bg-gradient-to-b from-indigo-500 to-indigo-600 hover:from-indigo-400 hover:to-indigo-500 text-white font-bold text-sm py-3.5 px-4 rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.2)] border border-indigo-500/50 hover:shadow-[0_0_25px_rgba(79,70,229,0.4)] flex items-center justify-center gap-1.5 transition-all cursor-pointer"
          >
